define(["dojo/text",
		...
		"dojo/_base/lang",
		"dojo/dom-class",
		"dojo/text!./templates/dGrowl.css",
		"dojo/text!./templates/dGrowl.html"],
function(t, declare, base, templated, topic, domCon, NotificationNode, query, lang, domClass, css, template) {
	return declare('dGrowl', [base, templated], {
		...
		'buildRendering': function() {
			...
			domCon.place("<style>" + css + "</style>", query('head')[0]);
		},
		'postCreate': function() {
			this.inherited(arguments);
			...
			// event
			this._s = topic.subscribe(this.topic, lang.hitch(this, this._subscribedChannels));
			// style myself..
		...
		'addChannel': function(c, o) {
			...
			this.chanDefs[c].node = domCon.create('li', {
				'class': "dGrowl-channel dGrowl-channel-" + c
			}); // node
		...
		'addNotification': function(m, o) {
			...
			else if (o.channel == undefined) o.channel = this.defaultChannel;
